...
7. Evaluation is noisy, so if the score improved (higher), rerun evaluation with a different seed and more runs per dataset to confirm the improvement is not just noise: `python evaluate.py --seed 43 --n-runs 10 > run2.log 2>&1`. Read out the score the same way.
